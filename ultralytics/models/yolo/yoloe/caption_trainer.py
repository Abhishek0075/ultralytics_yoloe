...
class YOLOECaptionDataset:
    ...
    This class handles loading captions from files named 'image-name_caption.txt'
    and provides them for text embedding generation.
    """
    ...
    def preprocess_batch(self, batch):
        ...
        if self.caption_dataset:
            ...
            if hasattr(self, 'text_embeddings') and self.text_embeddings:
                ...
                txt_feats = torch.stack([self.text_embeddings.get(caption, self.text_embeddings[list(self.text_embeddings.keys())[0]]) 
                                       for caption in captions]).to(self.device)
            else:
                # Generate embeddings on the fly (less efficient)
                if self.model is not None:
                    txt_feats = de_parallel(self.model).get_text_pe(captions, len(captions), without_reprta=True, cache_clip_model=False)
